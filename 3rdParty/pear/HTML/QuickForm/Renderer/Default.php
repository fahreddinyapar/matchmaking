...
class HTML_QuickForm_Renderer_Default extends HTML_QuickForm_Renderer
{
    ...
    */
    function renderHeader(&$header)
    {
	    ...
	    // Ozan
            $a = str_replace('{name}', $header->getName(), $this->_templates[$name]);
            $this->_html .= str_replace('{header}', $header->toHtml(), $a);
        } else {
            $a = str_replace('{name}', $header->getName(), $this->_headerTemplate);
            $this->_html .= str_replace('{header}', $header->toHtml(), $a);
        }
    ...
    */
    function renderElement(&$element, $required, $error)
    {
        if (!$this->_inGroup) {
            ...
            $a = str_replace('{name}', $element->getName(), $html);
            $this->_html .= str_replace('{element}', $element->toHtml(), $a);
